  ...
  g.each(function() {
    ...
    var legendItems = li.selectAll('g.legend-item')
      ...
      .on('mouseenter', function() {
        ...
        d3.selectAll('path.spectrum_nval').classed('unfocus', true);
        d3.select('path#spectrum_nval'+index).classed('unfocus', false).classed('focus', true);
        d3.select('g.hover-line').style('opacity', 0);
      })
      .on('mouseleave', function() {
        var index = d3.select(this).attr('data-index');
        d3.select(this).select('text').style('font-weight', 'normal');
        d3.selectAll('path.spectrum_nval').classed('unfocus', false);
        d3.select('path#spectrum_nval'+index).classed('focus', false);
        d3.select('g.hover-line').style('opacity', 1);
      });
