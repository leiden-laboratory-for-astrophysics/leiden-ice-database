from application import db, data_path, data_path_optc
...
def download_optc(url):
  filename = url.split('/')[-1]
  target_file = op.join(data_path_optc, filename)

  # Setup caching
  md5 = hashlib.md5(url.encode('utf-8')).hexdigest()
  cache_dir = op.join(data_path_optc, '..', 'cache')
  try:
    os.mkdir(cache_dir)
  except OSError:
    pass
...
# Add single optical constant
def add_optc(n_analogue, n_spectrum, temperature = 15):
  filename = download_optc(n_spectrum) if n_spectrum[0:4] == 'http' else n_spectrum

  db.session.add(N_optc(
    n_analogue_id = n_analogue if type(n_analogue) is int else n_analogue.id,
    path = filename,
    temperature = temperature
  ))

# Add optical constant to analogue
...
def fetch_N():
  ...
  n_user_id = db.session.query(N_User).first().get_id()
  t_start = time.time()
  ...
  print('VN is', vn)
  for i,j in zip(vn,T):
    n_val.append('https://icedb.strw.leidenuniv.nl/spectrum/download/' + str(i) + '/' + str(i) + '_' + str(j) + '.' + str(0) + 'K.txt')

  ##ADDINT TO OPTC FOLDER
  temperature = 165
  add_optical_const(n_analogue, n_val, temperature)
